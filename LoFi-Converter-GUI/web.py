    ...
    if isDownlaodable(url):
        try:
            ...
            if not os.access(FFMPEG_PATH, os.X_OK):
                raise Exception(f"FFmpeg at {FFMPEG_PATH} is not executable")
                
            print(f"FFmpeg path: {FFMPEG_PATH} (exists: {os.path.exists(FFMPEG_PATH)})")
            print(f"FFmpeg is executable: {os.access(FFMPEG_PATH, os.X_OK)}")
